function [uniformImage,profile,er] = csapsCorrect(raw_image)
%
...
for ii = 1
    ...
    colorChannel = imresize(grayImage,1/2,'bicubic'); % estimation is done on 1/4th sized image
	...
	%x = [x1d y1d];
    x = {1:size(z,1),1:size(z,2)}
	%% estimation step
    
    smoother = csapi(x,z);
    figure, surf(x{1},x{2},smoother.'), axis off
    %figure,plot3(x(1,:),x(2,:),z1d);
    %hold on,plot3(val(1,:),val(2,:),z1d,'r-')
    legend('smoothing spline','noisy data'), hold off    
	
    modeledColorChannel = reshape(zg, [rows columns]);
    modeledColorChannel = imresize(modeledColorChannel,2,'bicubic');
